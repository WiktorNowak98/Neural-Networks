...
for k in _solver:
    for i in _warstwy:
        ...
        for j in _eta:
            ...
            mlp = MLPClassifier(hidden_layer_sizes=(i,), max_iter=500, solver=k, verbose=0, random_state=1,
            ...
            mlp.score(dane_treningowe_std, etykiety_treningowe), mlp.score(dane_testowe_std, etykiety_testowe)))

            if k == 'sgd':
                loss_sgd.append(mlp.loss_curve_)
            if k == 'adam':
                loss_adam.append(mlp.loss_curve_)

# Wyznaczenie wykresów funkcji strat dla sgd oraz adama
        ...
        if k == 'adam':
            plt.plot(loss_adam[0], label='eta = 0.1')
            plt.plot(loss_adam[1], label='eta = 0.01')
            plt.plot(loss_adam[2], label='eta = 0.001')
            plt.ylabel('Strata obliczeniowa')
            plt.xlabel('Iteracja')
            plt.title('Solver: adam, Ilosc warstw: %d' % i)
            plt.legend()
            plt.grid()
            plt.show()
